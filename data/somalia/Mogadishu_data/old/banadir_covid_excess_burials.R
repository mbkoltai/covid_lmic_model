...
### Estimating excess burials across Banadir region
  ...
  ## Identify period of data availability for all cemeteries
    ...
    # dim(obs): 10380,35 | dim(x1): 1475,4 | dim(x2): 
  ...
  ## Sum each cemetery time series into a single one for Banadir
    ...
    obs$graves_best_ipol[is.na(obs$graves_best_ipol) & !is.na(obs$graves_best)]=
      obs$graves_best[is.na(obs$graves_best_ipol) & !is.na(obs$graves_best)]

 ...
 #...................................   
  ...
  ## Predictive model of graves as a function of surface area
    ...
    # GAMLSS generalised additive mixed model; ln(new surface area) as a growth variable
      ...
      x1 <- c("new_graves_since", "cemetery", "new_area_since")
      ...
      plot
      ggsave("new_graves_imputation.png", width = 18, height = 14, units = "cm", dpi = "print")

      # save final model
      filename <- "out_graves_area_pred_model.csv"
      write.csv(summary(fit), filename)
      
    # # Discrete generalised linear mixed model; log(starting n of graves) as offset, predictors = days since first observation, area at start
    #   # exclude observations with no new graves and two outliers with > 1500 new graves (very influential)
    #   x1 <- subset(obs, new_graves_since %in% c(2:1500))
    #   
    #   # new graves (more robust than graves as it constrains total predicted graves to >= 0, as it must be)
    #   fit <- f_glmm("new_graves_since", c("days_since_start_cat", "area_start_cat"), "(1 | cemetery)", "new_area_since", x1, "nbinom1")
    #   x2 <- f_do(fit, f_diag_fit, f_cv, k_folds, TRUE, TRUE)
    # 
    #   # plot CV output
    #   plot <- ggplot(x2) +
    ...
    #   # save final model
    #   filename <- "out_graves_area_pred_model.csv"
    #   write.table("Generalised linear mixed model to impute missing observations for new graves:", filename, sep = ",", col.names = FALSE, row.names = FALSE)
    #   write.table(tidy(fit, conf.int = TRUE), filename, sep = ",", row.names = FALSE, append = TRUE)
    #   write.table("--------------------", filename, sep = ",", col.names = FALSE, row.names = FALSE, append = TRUE)
    #   write.table(glance(fit), filename, sep = ",", row.names = FALSE, append = TRUE)
    #   write.table("--------------------", filename, sep = ",", col.names = FALSE, row.names = FALSE, append = TRUE)
    #  

...
ggplot(total_exp_death_preval_varied[truthvals,]) + geom_line(aes(x=prevalence*1e2,y=mean,linetype='expected (IFR)'),color='blue') +
  ...
  geom_text(data=best_approx_preval,aes(x=10^(log10(prevalence)+0.3)*1e2,y=mean/3.5,label=paste0(prevalence*1e2,"%")),size=insert_txt_size) + 
...
### read in modelling scenario analysis (modelling from Carl Pearson) --------------------

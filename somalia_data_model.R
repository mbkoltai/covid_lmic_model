...
lapply(c("tidyverse","deSolve","gtools","rstudioapi","wpp2019","countrycode","coronavirus","RcppRoll","scales","dttr2","wpp2019"),library,character.only=TRUE)
...
#' ### Age distribution
# "summary_by_age_grp.csv"
# "cases_by_age_gender.csv"
# "death_by_age_gender.csv"
...
cemet_dates=seq(as.Date("2019-09-01"),as.Date("2020-08-01"),14)
newsurface_biweekly=data.frame(date=seq(as.Date("2019-09-01"),as.Date("2020-08-01"),14),
           value_surf=c(rep(580,5),rep(650,6),rep(1000,3),1150,rep(1300,3),rep(900,3),750,rep(600,2)),datasource="satellite")
newsurface_biweekly[,"value_norm"] = (newsurface_biweekly$value_surf-min(newsurface_biweekly$value_surf))/(max(newsurface_biweekly$value_surf)-min(newsurface_biweekly$value_surf))

ggplot(newsurface_biweekly,aes(x=date,y=value_norm)) + geom_line() + theme_bw() + standard_theme + 
  scale_x_date(date_breaks="2 weeks") + ggtitle("new surface area index of Mogadishu cemeteries (m2)")

# compare to reported deaths
biweekly_deaths_sel=data.frame((subset(covid_somal,name %in% "death") %>% 
          mutate(sum_biweekly=roll_sum(cases,14,fill=NA,align="right")))[,c("date","sum_biweekly")],datasource="reported")
colnames(biweekly_deaths_sel)[2]="value"
biweekly_deaths_sel[,"value_surf"]=(max(newsurface_biweekly$value_surf)-min(newsurface_biweekly$value_surf))*(
    biweekly_deaths_sel$value/max(biweekly_deaths_sel$value,na.rm=T) ) + min(newsurface_biweekly$value_surf)
biweekly_deaths_sel[,"value_norm"]=
  (biweekly_deaths_sel$value-min(biweekly_deaths_sel$value,na.rm=T))/(max(biweekly_deaths_sel$value,na.rm=T)-min(biweekly_deaths_sel$value,na.rm=T))

# plot together
ggplot(bind_rows(biweekly_deaths_sel[,c("date","value_norm","datasource")],newsurface_biweekly),
    aes(x=date,y=value_norm,group=datasource,color=datasource)) + geom_line() + geom_point(aes(shape=datasource)) + theme_bw() + standard_theme +
    scale_x_date(date_breaks="2 weeks") + ggtitle(expression(paste("new surface area index (m2) - reported deaths (rescaled)")))
# SAVE
...
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
### simple SIR model for Somalia satellite data ----------------------
# ODEs
simple_sir_somalia <- function(t,x,parms) {
  S<-x[1]; E<-x[2]; I<-x[3]; I_hid=x[4]; R=x[5]; D=x[6]; newinfvar=x[7]
  with(as.list(c(x,parms)), {
    dS=-beta_dyn*S*I; dE=beta_dyn*S*I-sigma*E; 
    dI=sigma*E-gamma*I; dI_hid=gamma*I-(d_death/IFR_estim)*I_hid # infect who'll recover or die
    dR=d_death*((1-IFR_estim)/IFR_estim)*I_hid; dD=d_death*I_hid; d_newinfvar=sigma*E
    d_beta=0; # if (t<100) {print(c(t,beta_dyn))}
    return(list(c(dS,dE,dI,dI_hid,dR,dD,d_newinfvar,d_beta))) }) # 
}
...
eventfun <- function(t,y,parms){with (as.list(c(y,parms)),{
  ...
  return(c(S,E,I,I_hid,R,D,newinfvar,beta_dyn)) }) }
...
# var names
sir_varnames=c("S","E","I","I_hid","R","D","newinfvar"); 
# duration of simul
...
d_death=1/21; sigma=1/d_e; gamma=1/d_c; R0=(beta*mogadishu_popul)/gamma
# IFR
...
# RUN & process output
num_params=c(beta=beta_val,gamma=gamma,d_death=d_death,IFR=IFR_estim*3)
df_ode_solution=fcn_somal_sir_singlesimul(sir_varnames,var_categ_list,timesteps,day0=introd_date_scanvals[introd_date_scanvals %in% day0],
    OxCGRT_input$date,num_params,seed_size_val=seed_size_val,OxCGRT_input,compliance=compliance_val,
...
### plot one simulation -----
# plot timecourse for one param set
singlesimul<-fcn_somal_sir_singlesimul(sir_varnames,var_categ_list,timesteps,day0=introd_date_scanvals[9],OxCGRT_input$date,
      num_params=c(0.5,gamma,d_death,IFR_estim),seed_size_val=20,OxCGRT_input,compliance=0.5,N_tot*mogadishu_popul/sum(N_tot),
      xlimvals=c(60,300),"plot")
# plot
singlesimul[[2]] # singlesimul[[1]]
# SAVE
filetag=paste0(paste0(paste0(names(num_params)[c(1,4)],"_",round(num_params,3)[c(1,4)]),collapse="_"),paste0("_compliance",compliance_val))
...
# ESTIMATE of death rate for mogadishu: 0.4 deaths/10K ppl/day -> Mogadishu is 2.2M -> 88 deaths/day -> 1200/2 weeks
